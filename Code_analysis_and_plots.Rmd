...
#setwd("~/GitHub/CBD-coreBM/Normalisation_SCFAs/")
```

## BACTERIAL COUNT ANALYSIS

### Figure 2. Bacterial count test

Bacterial count (bacteria/µL) of stool samples: analysis

```{r}
#Flow cytometry tests --> Bacterial count (bacteria/µL)
bct_count_test <- read_csv("Bacterial_count_tests.csv", col_names = TRUE, col_types = NULL)
...
#Plot FIGURE 2
figure_2_plot <- ggplot(bct_count_test, aes(x = Dilution, y = Bacterial_count, colour = Sample_name)) +
...
#ggsave("Figures/Figure_2_bct_test.jpeg", width = 16, height = 9, units = "in", dpi = 300)
```

### Figure 3. Density plot bacterial count
...
total_bacterial_count <- read_csv("Bacterial_count_total.csv", col_names = TRUE, col_types = NULL)
...
#Plot FIGURE 3
figure_3_plot_outlier <- ggplot(total_bacterial_count, aes(x = bacterial_count, fill = patient_type, label = label_sample)) + 
...
figure_3_plot <- ggplot(bacterial_count, aes(x = bacterial_count, fill = patient_type, label = label_sample)) + 
  geom_density(alpha = 0.5, color = "dimgray", linewidth = 0) + 
  geom_textdensity(linewidth = 1, color = "gray30", size = 8, hjust = 0.36, spacing = 40) +
  labs(x = "Bacterial count (bacteria/µL)", y = "Relative frequency") +
  scale_x_continuous(label = scientific) + 
  scale_y_continuous(label = scientific) +
  scale_fill_manual(values = c("indianred3", "slategray3")) +
  theme_minimal() + 
...
#ggsave("Figures/Figure_3_bct_density.jpeg", width = 16, height = 9, units = "in", dpi = 300)
```
...
#Signifficant differences --> p-value = 4.328e-05
```
...
```{r}
stability_cdiff <- read_csv("Cdiff_sample_stability.csv", col_names = TRUE, col_types = NULL) 
...
#p-value of 0.615 (acetate), 0.04 (propionate), 0.457 (butyrate); no significant differences between groups (alfa = 0.01).
```
...
scfa_levels_hd <- read_csv("SCFA_levels_ugmL_HD.csv") 
#SCFAs levels (µg/mL) DYSBIOTIC PATIENTS
scfa_levels_cdiff <- read_csv("SCFA_levels_ugmL_CDIFF.csv") 
```
...
#BOXPLOT ACETATE
figure_4A_plot <- ggplot(acetate_data, aes(x = scfa, y = scfa_concentration, colour = patient_type)) +
  facet_wrap(~factor(normalisation, levels = c("not_normalised", "normalised_weight", "normalised_bct")), labeller = as_labeller(c("not_normalised" = "Not normalised", "normalised_bct" = "Normalised by bacterial count", "normalised_weight" = "Normalised by fresh weight")), scales = "free_y") +
  ...
  geom_point(position = position_dodge(width = 0.5), aes(group = patient_type), alpha = 0.5, size = 3.5) +
...
#ggsave("Figures/Figure_4A_boxplot_acetate.jpeg", width = 16, height = 9, units = "in", dpi = 300)
```
...
#BOXPLOT PROPIONATE
figure_4B_plot <- ggplot(propionate_data, aes(x = scfa, y = scfa_concentration, colour = patient_type)) +
  facet_wrap(~factor(normalisation, levels = c("not_normalised", "normalised_weight", "normalised_bct")), labeller = as_labeller(c("not_normalised" = "Not normalised", "normalised_bct" = "Normalised by bacterial count", "normalised_weight" = "Normalised by fresh weight")), scales = "free_y") +
...
#ggsave("Figures/Figure_4B_boxplot_propionate.jpeg", width = 16, height = 9, units = "in", dpi = 300)
```
...
#BOXPLOT BUTYRATE
figure_4C_plot <- ggplot(butyrate_data, aes(x = scfa, y = scfa_concentration, colour = patient_type)) +
  facet_wrap(~factor(normalisation, levels = c("not_normalised", "normalised_weight", "normalised_bct")), labeller = as_labeller(c("not_normalised" = "Not normalised", "normalised_bct" = "Normalised by bacterial count", "normalised_weight" = "Normalised by fresh weight")), scales = "free_y") +
...
#ggsave("Figures/Figure_4C_boxplot_butyrate.jpeg", width = 16, height = 9, units = "in", dpi = 300)
```
...
figure_5A_plot <- ggplot(acetate_data_bct, aes(x = scfa_concentration, fill = patient_type, label = labels_density)) + 
  ...
  labs(x = "Acetate levels normalised by bacterial count (µg/mL)", y = "Relative frequency", fill = NULL, tag = "A)") +
...
#ggsave("Figures/Figure_5A_density_acetate.jpeg", width = 16, height = 9, units = "in", dpi = 300)
```
...
figure_5B_plot <- ggplot(propionate_data_bct, aes(x = scfa_concentration, fill = patient_type, label = labels_density)) +
  geom_density(alpha = 0.5, color = "dimgray", linewidth = 0)  +
  ...
  labs(x = "Propionate levels normalised by bacterial count (µg/mL)", y = "Relative frequency", fill = NULL, tag = "B)") +
...
#ggsave("Figures/Figure_5B_density_propionate.jpeg", width = 16, height = 9, units = "in", dpi = 300)
```
...
figure_5C_plot <- ggplot(butyrate_data_bct, aes(x = scfa_concentration, fill = patient_type, label = labels_density)) + 
  ...
  labs(x = "Butyrate levels normalised by bacterial count (µg/mL)", y = "Relative frequency", fill = NULL, tag = "C)") +
  scale_x_continuous(limits = c(0, 250), breaks = c(0, 50, 100, 150, 200, 250)) +
...
#ggsave("Figures/Figure_5C_density_butyrate.jpeg", width = 16, height = 9, units = "in", dpi = 300)
```
...
figure_5D_plot <- ggplot(scfa_bct_normalised, aes(d = roc_outcome, m = scfa_concentration, color = scfa)) + 
  ...
  scale_color_manual(values = c("acetate" = "#440154FF", "propionate" = "khaki3", "butyrate" = "#24868EFF"),
                    ...
                    labels = c("Acetate - AUC = 0.696", "Propionate - AUC = 0.755", "Butyrate - AUC = 0.860")) + 
...
#ggsave("Figures/Figure_5D_ROC_curve.jpeg", width = 16, height = 9, units = "in", dpi = 300)

#Calculate the AUC of each SCFA (they are the same as pROC)
auc_curve <- round(calc_auc(figure_5D_plot)["AUC"], 3)
auc_curve
```
...
#ROC curve with ggplot (geom_roc)
figure_S1_plot <- ggplot(scfa_combined, aes(d = roc_outcome, m = combined_scfa, colour = scfa)) + 
  geom_roc(n.cuts = 0, labels = FALSE, linealpha = 0.7, size = 2)  + 
...
#Calculate the AUC of each SCFA (they are the same as pROC)
auc_curve_combined <- round(calc_auc(figure_S1_plot)["AUC"], 3)
auc_curve_combined

#ggsave("Figures/Figure_S1_combined_ROC.jpeg", width = 16, height = 9, units = "in", dpi = 300)
```
